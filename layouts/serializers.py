...
class LayoutItemSerializer(serializers.ModelSerializer):
    class Meta:
        model  = LayoutItem
        fields = [
            "id", "floor", "zone", "type", "name",
            "x", "y", "width", "height", "rotation",
            "shape", "z_index", "meta", "is_active",
...
class FloorCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_branch(self, value):
        if not can_manage_branch_layout(self.context["request"].user, value):
            raise serializers.ValidationError(
                "Bu branch layoutini boshqarishga ruxsat yo'q."
            )
        return value
...
class ZoneCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_floor(self, value):
        ...
        if not can_manage_branch_layout(user, value.branch):
            raise serializers.ValidationError(
                "Bu floor ga zone qo'shishga ruxsat yo'q."
            )
        return value
...
class LayoutItemCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_floor(self, value):
        if not can_manage_branch_layout(self.context["request"].user, value.branch):
            raise serializers.ValidationError(
                "Bu floor layoutini boshqarishga ruxsat yo'q."
            )
        return value
...
class PublicLayoutItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = LayoutItem
        fields = [
            "id", "floor", "zone", "zone_name",
            "type", "name", "x", "y",
            "width", "height", "rotation", "shape", "z_index", "meta",
        ]